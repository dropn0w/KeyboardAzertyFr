...
*** UPDATE => In their previous version, when you need to import the library keyboard.h (french version) and execute the payload on Arduino, a error accurs with the Keyboard.cpp file. 
They forgot to add something in one line, I corrected it. Everything is functionnal now.

You can just copy those files and past them on your arduino/libraries/Keyboard file.
